...
<body>
	...
	<div class="row" style="margin-top: 100px;">
		...
		<div id="divTab" class="col-lg-8">
			...
			foreach($result as $e){ 
				if($count=="0"){
					$tbodyid="tbody".$e['pbid'];?>
					<div class="text-right">
						<?php if (empty($e['arrive'])){?>
				    	<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalArrive" style="margin-bottom: 10px;margin-top: 20px" onclick="arrive(<?php echo "'".$e['phid']."','".$e['pbid']."'"?>)">
				    	<?php } else{?>
				    	<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalArrive" style="margin-bottom: 10px;margin-top: 20px" onclick="arrive(<?php echo "'".$e['phid']."','".$e['pbid']."'"?>)" disabled>
				    	<?php } ?>
						  確認到貨
						</button>
					</div>
						...
						<thead class="thead-dark">
							...
							<tbody id= <?php echo "\"". $tbodyid ."\""?>>
								<tr>
									...
									<td>
										<?php echo '<form method="POST" action="?action=Purchase_ContentCancel&phid='.$e['phid'].'&pbid='.$e['pbid'].'&chid='.$e['chid'].'">'; ?>
									    	<?php if (empty($e['arrive'])){?>
									...
									<td>
								    	...
								    	<button type="submit" class="btn btn-primary" disabled>
								    	<?php } else{?>
								    	<button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#ModalReturn" onclick="Preturn(<?php echo "'".$e['phid']."','".$e['pbid']."','".$e['chid']."','".$e['unit_price']."','".$e['total_price']."','".$e['quantity']."','".$e['item']."'"?>)">
								    	<?php } ?>
										  退貨
										</button>
									</td>
			 ...
			 <?php 
				} elseif ($count!="0" AND $pb==$e['pbid']) {
					if (empty($e['arrive'])){//還沒到貨
						echo "<script>$('#".$tbodyid."').append(\"<tr><th scope='row'>". $e['item'] ."</th><td>". $e['chid'] ."</td><td>". $e['unit_price'] ."</td><td>". $e['quantity'] ."</td><td>". $e['total_price'] ."</td><td>". $e['remark'] ."</td><td><form method='POST' action='?action=Purchase_ContentCancel&phid=".$e['phid']."&pbid=".$e['pbid']."&chid=".$e['chid']."'><button type='submit' class='btn btn-primary'>取消</button></form></td><td><button type='submit' class='btn btn-primary' disabled>退貨</button></td></tr>\");</script>";
					}else{//到貨
						if ($e['preturn']==1) {//已經退貨過
							echo "<script>$('#".$tbodyid."').append(\"<tr><th scope='row'>". $e['item'] ."</th><td>". $e['chid'] ."</td><td>". $e['unit_price'] ."</td><td>". $e['quantity'] ."</td><td>". $e['total_price'] ."</td><td>". $e['remark'] ."</td><td><form method='POST' action='?action=Purchase_ContentCancel&phid=".$e['phid']."&pbid=".$e['pbid']."&chid=".$e['chid']."'><button type='submit' class='btn btn-primary' disabled>取消</button></form></td><td><button type='submit' class='btn btn-primary' disabled>退貨</button></td></tr>\");</script>";
						}else{//還沒退貨過
							...
							echo "<script>document.getElementById(\"btReturn".$count."\").onclick=function(){Preturn('".$e['phid']."','".$e['pbid']."','".$e['chid']."','".$e['unit_price']."','".$e['total_price']."','".$e['quantity']."','".$e['item']."')};</script>";
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title" id="ModalLabelArrive">確認到貨</h5>
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	          <span aria-hidden="true">&times;</span>
	        </button>
	      </div>
	      <div class="modal-body">
	         <form method="POST" action="?action=Purchase_arrive">
	...
	<div class="modal fade" id="ModalReturn" tabindex="-1" role="dialog" aria-labelledby="ModalLabelReturn" aria-hidden="true">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	         <form method="POST" action="?action=Purchase_return">
	         	  <input  class="form-control" id="inputReturnPh" name="inputReturnPh" style="display: none;">
				  ...
				  <div class="form-group row">
				    <label for="inputRacceptance" class="col-sm-4 col-form-label">經辦人</label>
				    <div class="col-sm-8">
