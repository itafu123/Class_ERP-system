...
<body>
	...
	<div id="up_user_modal" class="modal4">
		  ...
		  <div class="modal-content">
			    ...
			    <form method="POST" action="?action=staff_change" id="staff_change_form">
	      			<div id="chdiv">
	      				uid：<t name="change_uid" id="change_uid"></t><br/> <br/>
	      				<input name="change_name" id="change_name" type="text"/>
	      				uname：<input name="uid" id="uid" type="text"/><br/> <br/>
	      				tel: <input name="change_tel" id="change_tel" type="text"/><br/> <br/>
	      				salary: <input name="change_salary" id="change_salary" type="text"/><br/> <br/>
	      				email: <input name="change_email" id="change_email" type="text"/><br/> <br/>
	      				<!-- did:<select name="did_select">
	      						<option value="0" selected="selected">Select did</option>
	      					</select>
	      				pid:<select name="pid_select">
	      						<option value="0" selected="selected">Select pid</option>
	      					</select> -->
	...
	<!-- The 新增角色Modal -->
	<div id="add_position_modal" class="modal5">
		  <!-- Modal content -->
		  <div class="modal-content">
			    <span class="close5">&times;</span>
			    <form method="POST" action="?action=staff_add_position" id="staff_add_position_form">
	      			角色名稱：<input name="character_name" type="text"/><br/> <br/>
	      			<input name="did" type="text" id="character_did"/>
      			<button class="btn btn-primary">確認新增</button>
	      		</form>
		  </div>
	</div>
	...
	<?php foreach($result as $r) {
		...
		$prepareSQL = "select pid,pname from position where did=:did";
		...
		?>
          ...
          <div id=<?php echo $r['did']?> class="tabcontent tabcontentBig">
				  ...
				  <?php if($pidresult){?>
						...
						<?php foreach($fidfname_result as $f) {
								if($f['fid']=='f010'){ ?>
									<button type="button" class="btn btn-secondary btn-info btn-sm" name=<?php echo $r['did']?> value=<?php echo $r['did']?> onclick='add_character(this.value)'>新增角色</button>
								<?php }
						}?>
						...
						<?php foreach($pidresult as $e){?>
							<!-- Tab content -->
							<?php 
							$prepareSQL = "select * from user where did=:did and pid=:pid and employment=1";
									    ...
									    <?php if($uid_uname_result){
											foreach($uid_uname_result as $u){?>
												<tr>
													...
													<?php  if($upc==1){?>
 														<td>
															<button class="btn btn-light" id="bt_up_user" name=<?php echo $u['uid'] ?> value=<?php echo $r['did'].$e['pid']?> onclick='nowupuser(this.value,this.name)'>修改</button>
													    </td>
													<?php }?>
...
<script>
		...
		var modal5 = document.getElementById('add_position_modal');
		var span5 = document.getElementsByClassName("close5")[0];
		function add_character(clicked_value){
			alert("clicked_value"+clicked_value);
			document.getElementById('character_did').setAttribute("value",clicked_value);
		...
		<?php if(isset($_GET['onclick'])&&($_GET['onclick']=="nowupuser")&&isset($_GET['did'])&&isset($_GET['pid'])&&isset($_GET['uid'])){ ?>
				nowupuser("<?php echo $_GET['did'].$_GET['pid']; ?>","<?php echo $_GET['uid']; ?>");
		<?php } ?>

		function nowupuser(clicked_value,clicked_name){
			<?php if(isset($_GET['onclick'])&&($_GET['onclick']=="nowupuser")&&isset($_GET['did'])&&isset($_GET['pid'])&&isset($_GET['uid'])){ ?>
				modal4.style.display = "block";
				var chdiv = document.getElementById("chdiv");
				document.getElementById("uid").setAttribute("value","<?php echo $_GET['uid']; ?>");
				// document.getElementById("uid").style.display = "none";
				// chdiv.innerHTML = '';
				<?php $prepareSQL = "SELECT * FROM user where uid = :uid";
				$executeSQL = array(':uid' => $_GET['uid']);
				$udataresult = $model->getDataSQL($prepareSQL, $executeSQL);
				foreach($udataresult as $udata){?>
					document.getElementById("change_name").setAttribute("value","<?php echo $udata['uname']; ?>");
					document.getElementById("change_tel").setAttribute("value","<?php echo $udata['tel']; ?>");
					document.getElementById("change_salary").setAttribute("value","<?php echo $udata['salary']; ?>");
					document.getElementById("change_email").setAttribute("value","<?php echo $udata['email']; ?>");
				<?php }?>

			<?php }else{ ?>
				var upuserdid=clicked_value.substr(0, 4);
				// alert('upuser did:'+upuserdid);
				var upuserpid=clicked_value.substr(4, 4);
				// alert('upuser pid:'+upuserpid);
				// alert('upuser uid:'+clicked_name);
				window.location.href = "index.php?action=staff&onclick=nowupuser&did="+upuserdid+"&pid="+upuserpid+"&uid="+clicked_name;
			<?php } ?>          
		}
